...
定义结构体时可能会存在变体字段，变体字段的值可能由当前环境中的一些已知信息来决定。选择器的值必须是在一个结构体中定义的值组成的枚举类型。select中定义的枚举的每个元素都应该有一个case选项和它对应。case选项允许包含有限的fall-through：如果两个case选项紧挨着并且之间没有字段的话，那么它们则使用相同的字段。因此，在下面的例子中，“orange”和“banana”同时指向V2。注意这是TLS 1.2协议中的新语法。
...
五种加密的操作 - 数字签名，流式加密，块式加密，使用额外数据的授权加密（AEAD）和公钥加密，这些分别被设计成数字签名的，流式加密的，块式加密的， AEAD加密的和公钥加密的。一个字段的加密处理方式通过在字段类型声明前面添加一个合适的关键词来指定。加密key有当前session状态（查看第6.1节）。
...
TLS记录层使用秘钥散列消息认证码（MAC）来保护消息的完整性。本文档中的加密套件使用基于哈希函数的HMAC结构，这定义在[HMAC](https://www.rfc-editor.org/rfc/rfc5246.html#ref-HMAC)。如果需要其他的套件，相应的套件可能定义了它们自己的MAC结构。

此外，需要一种结构来扩充消息为分组数据以便构造密钥或者实现验证。伪随机函数（PRF）把输入当做原文，种子和身份标识并产生任意长度的输出。

在本节中，我们基于HMAC定义一个伪随机数。这个伪随机函数使用SHA-256哈希方法，此哈希方法同时应用在本文档和TLS1.2协议讨论期间本文档之前的TLS文档中的所有加密套件中。新的加密套件必须显式指定一个PRF，通常应该使用带SHA-256的TLS为随机函数或者一个更健壮的标准哈希函数。

首先，我们定义一个数据扩充函数：P_hash(secret, data)，其使用一个单独的哈希函数来扩充秘钥或者种子为任意长度的输出：
...
TLS记录协议是一个分层的协议。在每一层中，消息可能包含长度、描述、以及内容字段。记录协议处理消息以便传输，把数据分片组成更容易管理的块，选择是否压缩数据，应用MAC，加密，然后传输这个结果。接受数据的处理步骤则是解密、校验数据、解压缩、重新组装，然后交给上一层的客户端。

此文档中有四个协议使用记录协议，它们分别是：握手协议，报警协议，切换加密方式协议，应用层数据协议。为了允许TLS协议的扩展，其他的记录类型也可以被记录协议所支持。在TLS内容类型注册中心，新的记录内容类型值由IANA分配，在[第12节]（https://www.rfc-editor.org/rfc/rfc5246.html#section-12）有所描述。

协议的实现一定不能发送未在此文档中定义的记录类型，除非通过一些扩展来协商实现。

使用TLS协议的任何协议设计时必须小心处理任何可能通过它的攻击。作为一个实践方面的因素，这意味着协议设计者必须意识到TLS提供了哪些安全属性，以及未提供哪些安全属性，不能依赖于未提供的安全属性。

尤其注意记录的类型和长度是不被加密保护的，如果消息自身是敏感的，应用程序设计者可能需要采取措施来减少信息的泄露。
